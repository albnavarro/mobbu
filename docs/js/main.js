...
`;var Fh={},Dh={},Ks=()=>Fh,ye=()=>Dh,Vh=async()=>{Fh=await fetch("./data/common.json").then(e=>e.json()).then(e=>e).catch(e=>console.warn("Something went wrong.",e)),Dh=await fetch("./data/legend.json").then(e=>e.json()).then(e=>e).catch(e=>console.warn("Something went wrong.",e))};var $h={github:Lh},mb=({event:e})=>{let t=e.target;console.log(t);let{url:r}=t.dataset,o=Ue("page-transition");_e(o,"url",r);let{navigationIsOpen:s}=$.get();s&&($.set("navigationIsOpen",!1),$.emit("closeNavigation"))};function fb({delegateEvents:e}){let{header:t}=Ks(),{links:r}=t;return r.map(o=>{let{svg:s,url:n,internal:i}=o;return b`<li class="l-header__sidenav__item">
    ...
    `;var gb=()=>{let{navigationIsOpen:e}=$.get("navigationIsOpen");if($.set("navigationIsOpen",t=>!t),e){$.emit("closeNavigation");return}$.emit("openNavigation")},Bh=({onMount:e,html:t,delegateEvents:r})=>(e(({element:o})=>{$.watch("closeNavigation",()=>{c.useFrame(()=>{o.classList.remove("is-open")})}),$.watch("openNavigation",()=>{c.useFrame(()=>{o.classList.add("is-open")})})}),t`
        <button
    ...
    `);function xb({data:e,staticProps:t,setState:r,bindProps:o,bindEvents:s}){return e.map((n,i)=>{let{label:a,url:l,children:p,section:u,sectioName:m,scrollToSection:d}=n;return u?b`
                ...
                `:p?b`
                  ...
                  `}).join("")}var Hh=({html:e,staticProps:t,setState:r,bindProps:o,bindEvents:s})=>{let{navigation:n}=Ks();return $.watch("closeAllAccordion",()=>{r("currentAccordionId",-1)}),e`
    ...
    `};function Cb({children:e,staticProps:t,callback:r}){return e.map(o=>{let{label:s,url:n,scrollToSection:i}=o;return b`
            ...
            `}).join("")}var Gh=({onMount:e,html:t,getState:r,setState:o,staticProps:s,bindProps:n,watch:i})=>{let{children:a,headerButton:l,callback:p}=r(),{label:u,url:m}=l;return e(({refs:d})=>{let{content:f}=d;return Eo.subscribe(f),Eo.reset(f),i("isOpen",async T=>{if(await Eo[T?"down":"up"](f),$.emit("refreshScroller"),!T){let w=Ue("main_navigation");_e(w,"currentAccordionId",-1,!1)}}),()=>{}}),t`
...
`;var Wd=({groups:e,trails:t})=>{let r=180/Math.PI,o=window.innerWidth,s=window.innerHeight,n=0,i=0,a=0,l=0,p=!1,u=t.map(C=>C.querySelector("svg")),m=Z.createSpring({data:{x:0,y:0},stagger:{each:3,from:"start"}});t.forEach(C=>{m.subscribe(({x:P,y:M})=>{C.style.translate=`${P}px ${M}px`})});let d=Z.createSpring({data:{rotation:0},stagger:{each:8,from:"start"}});u.forEach(C=>{d.subscribeCache(C,({rotation:P})=>{C.style.rotate=`${P}deg`})});let f=Z.createTween({data:{opacity:0,scale:1.4}});t.forEach(C=>{f.subscribe(({scale:P,opacity:M})=>{C.style.scale=`${P}`,C.style.opacity=M})});let T=c.useResize(()=>{o=window.innerWidth,s=window.innerHeight}),g=c.useMouseMove(({client:C})=>{if(!p)return;let{x:P,y:M}=C,_=M-n,k=P-i;if(Math.abs(k)>10||Math.abs(_)>10){n=M,i=P;let q=Math.atan2(_,k)*r+180,W=Math.abs(a-q);W>180&&a<q&&(l-=360),W>180&&a>q&&(l+=360);let Y=q+l+90;d.goTo({rotation:Y}),a=q}m.goTo({x:P-o/2,y:M-s/2})}),w=Z.createTween({data:{opacity:0,scale:.95},duration:2e3,ease:"easeOutQuart",stagger:{waitComplete:!0,each:5,from:"center"}});e.forEach(C=>{w.subscribeCache(C,({scale:P,opacity:M})=>{C.style.scale=`${P}`,C.style.opacity=M})});let x=Ne.createAsyncTimeline({repeat:1}).createGroup().goTo(w,{opacity:1,scale:1}).goTo(f,{scale:1,opacity:1}).closeGroup();return{playIntro:async()=>x.play().then(()=>{p=!0}),destroy:()=>{w.destroy(),w=null,x.destroy(),x=null,m.destroy(),m=null,d.destroy(),d=null,f.destroy(),f=null,T(),g(),o=null,s=null,n=null,i=null,a=null,l=null,u=null}}};var I0=10,E0=async({playIntro:e})=>{await e()},k0=()=>[...new Array(I0).keys()].map((e,t)=>b`
            ...
            `).join(""),Bd=({onMount:e,html:t,getState:r,staticProps:o})=>{let s=re.mq("min","desktop"),{svg:n}=s?r():"",{child:i}=ye(),{source:a}=i;return e(({refs:l})=>{if(!s)return;let{trail0:p,trail1:u,trail2:m,trail3:d,trail4:f,trail5:T,trail6:g,trail7:w,trail8:x,trail9:C,black:P,body:M,bottom_green:_,collo:k,dark_shadow:V,gambe:q,green_top:W,head:Y,light_shadow:D,head_bg:L,body_bg:G,head_green:se,highlight:j}=l,N=Wd({groups:[P,M,_,k,V,q,W,Y,D,L,G,se,j],trails:[p,u,m,d,f,T,g,w,x,C]}),{playIntro:v,destroy:S}=N;return E0({playIntro:v}),()=>{S()}}),t`<div class="svg-child-container">
        <only-desktop></only-desktop>
