(() => {
  ...
  var getScrollFunction = ({ callback: callback2, pin, ease, useThrottle }) => {
    if (pin)
      return mobCore.useScrollImmediate(callback2);
    if (ease && useThrottle)
      return mobCore.useScrollThrottle(callback2);
