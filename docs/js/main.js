...
`;var If=({getState:e})=>{let{rawContent:t}=e();navigator.clipboard.writeText(t)};function Ef({sync:e,bindProps:t,setState:r,delegateEvents:o}){return g`
    ...
    `}var kf=async({setState:e,getState:t,codeEl:r,currentKey:o,updateScroller:s,goToTop:n,syncParent:i})=>{let{urls:a}=t(),p=a.find(({label:m})=>m===o)?.source;if(!p?.length)return;let u=g`<html-content
        ...
        ${i}
    ></html-content>`;r.insertAdjacentHTML("afterbegin",u),await po(r),e("rawContent",r.textContent),s(),n()},Rp=({codeEl:e,removeDOM:t})=>{e.textContent="";let r=e.firstElementChild;r&&t(r)},Pp=({onMount:e,setState:t,getState:r,repeat:o,html:s,bindProps:n,delegateEvents:i,staticProps:a,computed:l,watch:p,removeDOM:u,syncParent:m})=>(e(({element:d,refs:f})=>{let{screenEl:T,scrollerEl:b,codeEl:S,scrollbar:x}=f,{updateScroller:C,move:_,goToTop:W}=Ep({screen:T,scroller:b,scrollbar:x});x.addEventListener("input",()=>{_(x.value)}),A.watch("beforeRouteLeave",()=>{t("urls",[])}),l("currentButtonState",["urls","activeContent"],(k,$)=>(k.length>0?(d.classList.add("active"),document.body.style.overflow="hidden"):(d.classList.remove("active"),document.body.style.overflow="",Rp({codeEl:S,removeDOM:u}),t("activeContent",""),W()),$));let M=p("activeContent",k=>{Rp({codeEl:S,removeDOM:u}),kf({setState:t,getState:r,codeEl:S,currentKey:k,updateScroller:C,goToTop:W,staticProps:a,syncParent:m})});return()=>{M()}}),s`
...
`;var Qh=({html:e,onMount:t})=>(t(({refs:r})=>{let{shape:o}=r;c.useFrame(()=>{o.classList.add("active")})}),e`
    ...
    `);var Pb=E({name:"shape-right",component:Qh}),Ab=E({name:"shape-left",component:Kh});var je=({disableOffcanvas:e})=>{let t="OffscreenCanvas"in window&&!e;return{useOffscreen:t,context:t?"bitmaprenderer":"2d"}},ze=({useOffscreen:e,canvas:t})=>{let r=e?new OffscreenCanvas(t.width,t.height):null,o=e?r.getContext("2d"):null;return{offscreen:r,offScreenCtx:o}},He=({useOffscreen:e,offscreen:t,ctx:r})=>{if(e){let o=t.transferToImageBitmap();r.transferFromImageBitmap(o)}},yt=e=>"roundRect"in e,ed=(e,t,r,o,s,n)=>{o<2*n&&(n=o/2),s<2*n&&(n=s/2),e.beginPath(),e.moveTo(t+n,r),e.arcTo(t+o,r,t+o,r+s,n),e.arcTo(t+o,r+s,t,r+s,n),e.arcTo(t,r+s,t,r,n),e.arcTo(t,r,t+o,r,n),e.closePath()},Zr=({canvas:e,numberOfRow:t,numberOfColumn:r,cellWidth:o,cellHeight:s,gutter:n})=>[...new Array(t*r+t).keys()].reduce(i=>{let{row:a,col:l,items:p}=i,u=l<r?l+1:0,m=u===0?a+1:a,d=(o+n)*u,f=(s+n)*m;return{row:m,col:u,items:[...p,{width:o,height:s,x:d,y:f,centerX:d+o/2,centerY:f+s/2,offsetXCenter:pr({canvasWidth:e.width,width:o,gutter:n,numberOfColumn:r}),offsetYCenter:hr({canvasHeight:e.height,height:s,gutter:n,numberOfRow:t}),gutter:n,numberOfColumn:r}]}},{row:0,col:-1,items:[]}),pr=({canvasWidth:e,width:t,gutter:r,numberOfColumn:o})=>e/2-(t+r)*o/2-t/2,hr=({canvasHeight:e,height:t,gutter:r,numberOfRow:o})=>e/2-(t+r)*(o+1)/2-t/2;var td=({canvas:e,numberOfRow:t,numberOfColumn:r,cellWidth:o,cellHeight:s,gutter:n,fill:i,disableOffcanvas:a,stagger:l,reorder:p})=>{let{useOffscreen:u,context:m}=je({disableOffcanvas:a}),d=!0,f=[],T=[],b={},S={},x=e.getContext(m,{alpha:!1}),C="#000",_="#fff",{activeRoute:W}=A.get(),{offscreen:M,offScreenCtx:k}=ze({useOffscreen:u,canvas:e});e.width=e.clientWidth,e.height=e.clientHeight,f=Zr({canvas:e,numberOfRow:t,numberOfColumn:r,cellWidth:o,cellHeight:s,gutter:n}).items,T=p?f.map((L,J)=>({...L,scale:1,rotate:0,hasFill:i.includes(J)})).sort(L=>L.hasFill?-1:1).reverse():f.map((L,J)=>{let ne=i.includes(J);return{...L,scale:1,rotate:0,hasFill:ne}}),b=re.createTween({ease:"easeInOutQuad",stagger:l,data:{scale:1,rotate:0}}),T.forEach(L=>{b.subscribeCache(L,({scale:J,rotate:ne})=>{L.rotate=ne,L.scale=J})});let $=()=>{if(!x)return;u&&(M.width=e.width,M.height=e.height);let L=u?k:x;L.fillStyle="#1a1b26",L.fillRect(0,0,e.width,e.height),T.forEach(({x:J,y:ne,centerX:j,centerY:O,width:y,height:v,rotate:R,scale:P,hasFill:V,offsetXCenter:G,offsetYCenter:Z})=>{let Ie=Math.PI/180*R,ae=Math.cos(Ie)*P,de=Math.sin(Ie)*P;L.setTransform(ae,de,-de,ae,Math.round(j+G),Math.round(O+Z)),L.beginPath(),L.rect(Math.round(-j+J),Math.round(-O+ne),y,v),L.fillStyle=V?_:C,L.fill(),L.setTransform(1,0,0,1,0,0)}),He({useOffscreen:u,offscreen:M,ctx:x})};S=Be.createAsyncTimeline({repeat:-1,yoyo:!0}).label({name:"label1"}).goTo(b,{scale:1.5,rotate:90},{duration:1e3}).goTo(b,{scale:.5},{duration:500}).goTo(b,{rotate:180,scale:1.2},{duration:500}).goTo(b,{scale:1.3},{duration:500}).goTo(b,{scale:1},{duration:1200}),S.onLoopEnd(({direction:L,loop:J})=>{console.log(`loop end: ${L}, ${J}`)}),S.play();let U=()=>{$(),d&&c.useNextFrame(()=>U())};c.useFrame(({time:L})=>{U({time:L})});let B=c.useResize(()=>{e.width=e.clientWidth,e.height=e.clientHeight,T.forEach(L=>{let{width:J,height:ne,gutter:j,numberOfColumn:O}=L;L.offsetXCenter=pr({canvasWidth:e.width,width:J,gutter:j,numberOfColumn:O}),L.offsetYCenter=hr({canvasHeight:e.height,height:ne,gutter:j,numberOfRow:t})}),c.useFrame(()=>$())}),K=D.watch("openNavigation",()=>{S?.stop(),d=!1}),F=D.watch("closeNavigation",()=>setTimeout(async()=>{d=!0;let{activeRoute:L}=A.get();L===W&&(S?.play(),c.useFrame(()=>U()))},500));return()=>{b.destroy(),S.destroy(),B(),F(),K(),b=null,S=null,x=null,M=null,k=null,f=[],T=[],d=!1}};var rd=({onMount:e,html:t,getState:r,staticProps:o})=>{e(({refs:i})=>{if(oe.mq("max","desktop"))return;let{wrap:a,canvas:l}=i,p=td({canvas:l,...r()});return c.useFrame(()=>{a.classList.add("active")}),()=>{p()}});let{animatedPatternN0:s}=ve(),{source:n}=s;return t`
        <div>
    ...
    `};var _b=E({name:"animatedpattern-n0",component:rd,exportState:["numberOfRow","numberOfColumn","cellWidth","cellHeight","gutter","fill","stagger","reorder","disableOffcanvas"],state:{numberOfRow:()=>({value:10,type:Number}),numberOfColumn:()=>({value:10,type:Number}),cellWidth:()=>({value:65,type:Number}),cellHeight:()=>({value:65,type:Number}),gutter:()=>({value:1,type:Number}),fill:()=>({value:[16,27,38,49,60,71,82,93],type:Array}),stagger:()=>({value:{each:5,grid:{col:11,row:11,direction:"row"},waitComplete:!1},type:"any"}),reorder:()=>({value:!0,type:Boolean}),disableOffcanvas:!!(Re()||se())}});var od=({canvas:e,numberOfRow:t,numberOfColumn:r,cellWidth:o,cellHeight:s,gutter:n,fill:i,disableOffcanvas:a})=>{let{useOffscreen:l,context:p}=je({disableOffcanvas:a}),u=!0,m=[],d=[],f={},T={},b={},{top:S,left:x}=ie(e),C=e.getContext(p,{alpha:!1}),_="#000",W="#fff",{activeRoute:M}=A.get(),{offscreen:k,offScreenCtx:$}=ze({useOffscreen:l,canvas:e});e.width=e.clientWidth,e.height=e.clientHeight,m=Zr({canvas:e,numberOfRow:t,numberOfColumn:r,cellWidth:o,cellHeight:s,gutter:n}).items,d=m.map((O,y)=>({...O,scale:0,mouseX:0,mouseY:0,hasFill:i.includes(y)})).sort(O=>O.hasFill?-1:1),f=re.createLerp({data:{mouseX:0,mouseY:0}}),d.forEach(O=>{f.subscribeCache(O,({mouseX:y,mouseY:v})=>{O.mouseX=y,O.mouseY=v})}),T=re.createTween({ease:"easeInOutSine",stagger:{each:5,from:"center",waitComplete:!1},data:{scale:0}}),d.forEach(O=>{T.subscribeCache(O,({scale:y})=>{O.scale=y})});let U=()=>{if(!C)return;l&&(k.width=e.width,k.height=e.height);let O=l?$:C;O.fillStyle="#1a1b26",O.fillRect(0,0,e.width,e.height),d.forEach(({x:y,y:v,centerX:R,centerY:P,width:V,height:G,mouseX:Z,mouseY:Ie,scale:ae,hasFill:de,offsetXCenter:Ne,offsetYCenter:Fe})=>{let Me=Z-(e.width-(V+n)*r)/2,Ge=Ie-(e.height-(G+n)*t)/2,Je=(y-Me)/250,vt=(v-Ge)/250,St=Math.sqrt(Math.pow(Math.abs(Je),2)+Math.pow(Math.abs(vt),2)),Yt=Oe(Math.abs(St),0,2),eo=0,Ue=Math.cos(eo)*(Yt+ae),Lo=Math.sin(eo)*(Yt+ae);O.setTransform(Ue,Lo,-Lo,Ue,Math.round(R+Ne),Math.round(P+Fe)),O.beginPath(),O.rect(Math.round(-R+y),Math.round(-P+v),V,G),O.fillStyle=de?W:_,O.fill(),O.setTransform(1,0,0,1,0,0)}),He({useOffscreen:l,offscreen:k,ctx:C})};b=Be.createAsyncTimeline({repeat:-1,yoyo:!0}).goTo(T,{scale:.3},{duration:1e3}),b.play();let B=({x:O,y})=>{f.goTo({mouseX:O-x,mouseY:y-S})},K=c.useMouseMove(({client:O})=>{let{x:y,y:v}=O;B({x:y,y:v})}),F=c.useTouchMove(({client:O})=>{let{x:y,y:v}=O;B({x:y,y:v})}),L=()=>{U(),u&&c.useNextFrame(()=>L())};c.useFrame(({time:O})=>{L({time:O})});let J=c.useResize(()=>{e.width=e.clientWidth,e.height=e.clientHeight,S=ie(e).top,x=ie(e).left,d.forEach(O=>{let{width:y,height:v,gutter:R,numberOfColumn:P}=O;O.offsetXCenter=pr({canvasWidth:e.width,width:y,gutter:R,numberOfColumn:P}),O.offsetYCenter=hr({canvasHeight:e.height,height:v,gutter:R,numberOfRow:t})}),c.useFrame(()=>U())}),ne=D.watch("openNavigation",()=>{b?.stop(),u=!1}),j=D.watch("closeNavigation",()=>setTimeout(async()=>{u=!0;let{activeRoute:O}=A.get();O===M&&(b?.play(),c.useFrame(()=>L()))},500));return()=>{T.destroy(),b.destroy(),f.destroy(),J(),K(),F(),j(),ne(),T=null,b=null,f=null,C=null,k=null,$=null,m=[],d=[],u=!1}};var sd=({onMount:e,html:t,getState:r,staticProps:o})=>{e(({refs:i})=>{if(oe.mq("max","desktop"))return;let{wrap:a,canvas:l}=i,p=od({canvas:l,...r()});return c.useFrame(()=>{a.classList.add("active")}),()=>{p()}});let{animatedPatternN1:s}=ve(),{source:n}=s;return t`
        <div>
            <only-desktop></only-desktop>
            <code-button
                ${o({drawers:[{label:"description",source:n.description},{label:"definition",source:n.definition},{label:"component",source:n.component},{label:"animation",source:n.animation}],style:"legend",color:"green"})}
    ...
    `};var Fb=E({name:"caterpillar-n1",component:ld,isolateCreation:!0,isolateOnMount:!0,exportState:["numItems","width","height","fill","opacity","radius","rotationEach","centerEach","rotationDuration","disableOffcanvas"],state:{numItems:20,width:40,height:40,fill:[14],opacity:.05,radius:0,rotationEach:15,centerEach:3,rotationDuration:5e3,disableOffcanvas:!!(Re()||se())}});var Ii=({value:e,direction:t,isForced:r})=>{r||console.log(`current: ${e}, direction: ${t}`)},cd=({canvas:e,numItems:t,width:r,height:o,radius:s,fill:n,opacity:i,xAmplitude:a,yAmplitude:l,duration:p,friction:u,rotationDefault:m,disableOffcanvas:d})=>{let{useOffscreen:f,context:T}=je({disableOffcanvas:d}),b=!0,S=e.getContext(T,{alpha:!1}),x=[],C=m,{activeRoute:_}=A.get(),{offscreen:W,offScreenCtx:M}=ze({useOffscreen:f,canvas:e}),k=f?M:S,$=yt(k)&&!se();k=null,x=[...new Array(t).keys()].map((j,O)=>{let y=O>=t/2?t/2+(t/2-O):O,v=r+r/3*y,R=o+o/3*y,P=n.includes(O)?1:(t-O)*i;return{width:v,height:R,x:0,y:0,hasFill:n.includes(O),opacity:P,radius:s,rotate:0}}),e.width=e.clientWidth,e.height=e.clientHeight;let U=re.createSequencer({stagger:{each:6},data:{x:p/4,rotate:0},duration:p}).goTo({x:p+p/4},{start:0,end:p,ease:"easeLinear"}).goTo({rotate:()=>-C},{start:0,end:5,ease:"easeInOutBack"}).goTo({rotate:0},{start:5,end:p,ease:"easeInOutBack"}).label("mylabel",2).add(({isForced:j,direction:O})=>{Ii({isForced:j,direction:O,value:1})},1).add(({isForced:j,direction:O})=>{Ii({isForced:j,direction:O,value:5})},5).add(({isForced:j,direction:O})=>{Ii({isForced:j,direction:O,value:9})},9);x.forEach(j=>{U.subscribeCache(j,({x:O,rotate:y})=>{let v=O/u,R=2/(3-Math.cos(2*v)),P=R*Math.cos(v)*a,V=R*Math.sin(2*v)/2*l;j.x=P,j.y=V,j.rotate=y})});let B=Be.createSyncTimeline({repeat:-1,yoyo:!1,duration:4e3}).add(U);B.onLoopEnd(({loop:j,direction:O})=>{console.log(`loop end: ${j} , ${O}`)});let K=()=>{if(!S)return;f&&(W.width=e.width,W.height=e.height);let j=f?M:S;j.fillStyle="#1a1b26",j.fillRect(0,0,e.width,e.height),x.forEach(({width:O,height:y,x:v,y:R,radius:P,rotate:V,hasFill:G,opacity:Z})=>{let Ie=e.width/2,ae=e.height/2,de=1,Ne=Math.PI/180*V,Fe=Math.cos(Ne)*de,Me=Math.sin(Ne)*de;j.setTransform(Fe,Me,-Me,Fe,Ie+v,ae+R),$?(j.beginPath(),j.roundRect(Number.parseInt(-O/2),Number.parseInt(-y/2),O,y,[150,0])):(j.beginPath(),j.rect(Number.parseInt(-O/2),Number.parseInt(-y/2),O,y,P)),G?j.fillStyle="rgba(255, 255, 255, 1)":(j.fillStyle=`rgba(26, 27, 38, ${Z})`,j.strokeStyle=`rgba(255, 255, 255, ${Z})`,j.stroke()),j.fill(),j.setTransform(1,0,0,1,0,0)}),He({useOffscreen:f,offscreen:W,ctx:S})},F=()=>{K(),b&&c.useNextFrame(()=>F())};c.useFrame(()=>F()),B.play();let L=c.useResize(()=>{e.width=e.clientWidth,e.height=e.clientHeight,K()}),J=D.watch("openNavigation",()=>{b=!1,B?.pause()}),ne=D.watch("closeNavigation",()=>setTimeout(()=>{b=!0;let{activeRoute:j}=A.get();j===_&&(B?.resume(),c.useFrame(()=>F()))},500));return{destroy:()=>{b=!1,L(),J(),ne(),U.destroy(),B.destroy(),S=null,W=null,M=null,x=[]},play:()=>{B.stop(),B.play()},playReverse:()=>{B.stop(),B.playReverse()},playUseCurrent:()=>B.play({useCurrent:!0}),playReverseUseCurrent:()=>B.playReverse({useCurrent:!0}),playFromLabel:()=>{B.stop(),B.playFrom("mylabel")},plaFromLabelReverse:()=>{B.stop(),B.playFromReverse("mylabel")},stop:()=>B.stop(),pause:()=>B.pause(),resume:()=>B.resume(),reverse:()=>B.reverse(),setRotation:j=>C=j}};function Db({buttons:e}){return Object.entries(e).map(([t,r])=>{let{label:o}=r;return g` <li class="c-canvas__controls__item">
                <button
                    type="button"
