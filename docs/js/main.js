(() => {
  ...
  var MAIN_STORE_CONTENT_ID = "contentId";
  var MAIN_STORE_ROOT_ELEMENT = "rootElement";
  var MAIN_STORE_INDEX_PAGE = "index";
  var MAIN_STORE_PAGE_NOT_FOUND = "pageNotFound";
  var MAIN_STORE_COMPONENT_LIST = "componentList";
  ...
  var MAIN_STORE_BEFORE_PAGE_TRANSITION = "beforePageTransition";
  var MAIN_STORE_PAGE_TRANSITION = "pageTransition";
  ...
  var incrementTickQueuque = (props) => {
    const id2 = mobCore.getUnivoqueId();
    queuqe.set(id2, props);
    return () => queuqe.delete(id2);
  };
  ...
  var inizializeRepeat = ({ repeatId, repeatersParents }) => {
    if (!repeatId || !repeatersParents || repeatersParents.length === 0)
      return;
    const obj = repeatMap.get(repeatId);
    if (!obj)
      return;
    const containerList = repeatersParents.find(({ id: id2 }) => {
      return id2 === repeatId;
    });
  ...
  var loadRoute = async ({ route = "", params = {} }) => {
    ...
    if (!skip)
      mainStore.set(MAIN_STORE_AFTER_ROUTE_CHANGE, route);
...
Loading snippet ...</pre
  ...
  function hasAnchor({ id: id2 }) {
    return id2 && id2.length > 0;
  }
  var options = {
    root: null,
  ...
  var SpacerAnchor = async ({ html, getState, onMount }) => {
    ...
    onMount(({ element }) => {
      const shouldAddToAnchor = hasAnchor({ id: id2 });
      if (!shouldAddToAnchor)
        return;
      anchorStore.set("items", (val2) => {
        return [...val2, { id: id2, label, element }];
      });
...
`;
  ...
  var beforePageTransition = async ({ oldNode }) => {
