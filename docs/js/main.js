(() => {
  ...
  var update = ({ id, callBackObject, frame }) => {
    ...
    if (data3.has(frame + currentFrame2))
      return;
  ...
  var parseComponentsRecursive = async ({
    ...
    const { content: content2, componentParsed, id } = await userFunctionComponent(
...
`;
  ...
  var initTestMapStore = async () => {
    ...
    test.computed("computedProp2", ["myObject"], (myObject2) => {
      return `${myObject2.prop1}_`;
    });
