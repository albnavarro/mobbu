...
`;var kf=({getState:e})=>{let{rawContent:t}=e();navigator.clipboard.writeText(t)};function Rf({sync:e,bindProps:t,setState:r,delegateEvents:o}){return b`
    ...
    `}var Pf=async({setState:e,getState:t,codeEl:r,currentKey:o,updateScroller:s,goToTop:n,syncParent:i})=>{let{urls:a}=t(),p=a.find(({label:m})=>m===o)?.source;if(!p?.length)return;let u=b`<html-content
        ...
        ${i}
    ></html-content>`;r.insertAdjacentHTML("afterbegin",u),await po(r),e("rawContent",r.textContent),s(),n()},Pp=({codeEl:e,removeDOM:t})=>{e.textContent="";let r=e.firstElementChild;r&&t(r)},Ap=({onMount:e,setState:t,getState:r,repeat:o,html:s,bindProps:n,delegateEvents:i,staticProps:a,computed:l,watch:p,removeDOM:u,syncParent:m})=>(e(({element:d,refs:f})=>{let{screenEl:T,scrollerEl:g,codeEl:S,scrollbar:x}=f,{updateScroller:C,move:_,goToTop:V}=kp({screen:T,scroller:g,scrollbar:x});x.addEventListener("input",()=>{_(x.value)}),O.watch("beforeRouteLeave",()=>{t("urls",[])}),l("currentButtonState",["urls","activeContent"],(I,F)=>(I.length>0?(d.classList.add("active"),document.body.style.overflow="hidden"):(d.classList.remove("active"),document.body.style.overflow="",Pp({codeEl:S,removeDOM:u}),t("activeContent",""),V()),F));let P=p("activeContent",I=>{Pp({codeEl:S,removeDOM:u}),Pf({setState:t,getState:r,codeEl:S,currentKey:I,updateScroller:C,goToTop:V,staticProps:a,syncParent:m})});return()=>{P()}}),s`
...
`;var Qh=({html:e,onMount:t})=>(t(({refs:r})=>{let{shape:o}=r;c.useFrame(()=>{o.classList.add("active")})}),e`
    ...
    `);var _b=k({name:"shape-right",component:Qh}),Ob=k({name:"shape-left",component:Kh});var je=({disableOffcanvas:e})=>{let t="OffscreenCanvas"in window&&!e;return{useOffscreen:t,context:t?"bitmaprenderer":"2d"}},ze=({useOffscreen:e,canvas:t})=>{let r=e?new OffscreenCanvas(t.width,t.height):null,o=e?r.getContext("2d"):null;return{offscreen:r,offScreenCtx:o}},He=({useOffscreen:e,offscreen:t,ctx:r})=>{if(e){let o=t.transferToImageBitmap();r.transferFromImageBitmap(o)}},yt=e=>"roundRect"in e,ed=(e,t,r,o,s,n)=>{o<2*n&&(n=o/2),s<2*n&&(n=s/2),e.beginPath(),e.moveTo(t+n,r),e.arcTo(t+o,r,t+o,r+s,n),e.arcTo(t+o,r+s,t,r+s,n),e.arcTo(t,r+s,t,r,n),e.arcTo(t,r,t+o,r,n),e.closePath()},Zr=({canvas:e,numberOfRow:t,numberOfColumn:r,cellWidth:o,cellHeight:s,gutter:n})=>[...new Array(t*r+t).keys()].reduce(i=>{let{row:a,col:l,items:p}=i,u=l<r?l+1:0,m=u===0?a+1:a,d=(o+n)*u,f=(s+n)*m;return{row:m,col:u,items:[...p,{width:o,height:s,x:d,y:f,centerX:d+o/2,centerY:f+s/2,offsetXCenter:pr({canvasWidth:e.width,width:o,gutter:n,numberOfColumn:r}),offsetYCenter:hr({canvasHeight:e.height,height:s,gutter:n,numberOfRow:t}),gutter:n,numberOfColumn:r}]}},{row:0,col:-1,items:[]}),pr=({canvasWidth:e,width:t,gutter:r,numberOfColumn:o})=>e/2-(t+r)*o/2-t/2,hr=({canvasHeight:e,height:t,gutter:r,numberOfRow:o})=>e/2-(t+r)*(o+1)/2-t/2;var td=({canvas:e,numberOfRow:t,numberOfColumn:r,cellWidth:o,cellHeight:s,gutter:n,fill:i,disableOffcanvas:a,stagger:l,reorder:p})=>{let{useOffscreen:u,context:m}=je({disableOffcanvas:a}),d=!0,f=[],T=[],g={},S={},x=e.getContext(m,{alpha:!1}),C="#000",_="#fff",{activeRoute:V}=O.get(),{offscreen:P,offScreenCtx:I}=ze({useOffscreen:u,canvas:e});e.width=e.clientWidth,e.height=e.clientHeight,f=Zr({canvas:e,numberOfRow:t,numberOfColumn:r,cellWidth:o,cellHeight:s,gutter:n}).items,T=p?f.map((N,q)=>({...N,scale:1,rotate:0,hasFill:i.includes(q)})).sort(N=>N.hasFill?-1:1).reverse():f.map((N,q)=>{let Q=i.includes(q);return{...N,scale:1,rotate:0,hasFill:Q}}),g=Z.createTween({ease:"easeInOutQuad",stagger:l,data:{scale:1,rotate:0}}),T.forEach(N=>{g.subscribeCache(N,({scale:q,rotate:Q})=>{N.rotate=Q,N.scale=q})});let F=()=>{if(!x)return;u&&(P.width=e.width,P.height=e.height);let N=u?I:x;N.fillStyle="#1a1b26",N.fillRect(0,0,e.width,e.height),T.forEach(({x:q,y:Q,centerX:B,centerY:M,width:y,height:v,rotate:R,scale:A,hasFill:W,offsetXCenter:Y,offsetYCenter:ee})=>{let Ie=Math.PI/180*R,ae=Math.cos(Ie)*A,de=Math.sin(Ie)*A;N.setTransform(ae,de,-de,ae,Math.round(B+Y),Math.round(M+ee)),N.beginPath(),N.rect(Math.round(-B+q),Math.round(-M+Q),y,v),N.fillStyle=W?_:C,N.fill(),N.setTransform(1,0,0,1,0,0)}),He({useOffscreen:u,offscreen:P,ctx:x})};S=Ne.createAsyncTimeline({repeat:-1,yoyo:!0}).label({name:"label1"}).goTo(g,{scale:1.5,rotate:90},{duration:1e3}).goTo(g,{scale:.5},{duration:500}).goTo(g,{rotate:180,scale:1.2},{duration:500}).goTo(g,{scale:1.3},{duration:500}).goTo(g,{scale:1},{duration:1200}),S.onLoopEnd(({direction:N,loop:q})=>{console.log(`loop end: ${N}, ${q}`)}),S.play();let U=()=>{F(),d&&c.useNextFrame(()=>U())};c.useFrame(({time:N})=>{U({time:N})});let j=c.useResize(()=>{e.width=e.clientWidth,e.height=e.clientHeight,T.forEach(N=>{let{width:q,height:Q,gutter:B,numberOfColumn:M}=N;N.offsetXCenter=pr({canvasWidth:e.width,width:q,gutter:B,numberOfColumn:M}),N.offsetYCenter=hr({canvasHeight:e.height,height:Q,gutter:B,numberOfRow:t})}),c.useFrame(()=>F())}),G=$.watch("openNavigation",()=>{S?.stop(),d=!1}),D=$.watch("closeNavigation",()=>setTimeout(async()=>{d=!0;let{activeRoute:N}=O.get();N===V&&(S?.play(),c.useFrame(()=>U()))},500));return()=>{g.destroy(),S.destroy(),j(),D(),G(),g=null,S=null,x=null,P=null,I=null,f=[],T=[],d=!1}};var rd=({onMount:e,html:t,getState:r,staticProps:o})=>{e(({refs:i})=>{if(oe.mq("max","desktop"))return;let{wrap:a,canvas:l}=i,p=td({canvas:l,...r()});return c.useFrame(()=>{a.classList.add("active")}),()=>{p()}});let{animatedPatternN0:s}=ye(),{source:n}=s;return t`
        <div>
    ...
    `};var Nb=k({name:"animatedpattern-n0",component:rd,exportState:["numberOfRow","numberOfColumn","cellWidth","cellHeight","gutter","fill","stagger","reorder","disableOffcanvas"],state:{numberOfRow:()=>({value:10,type:Number}),numberOfColumn:()=>({value:10,type:Number}),cellWidth:()=>({value:65,type:Number}),cellHeight:()=>({value:65,type:Number}),gutter:()=>({value:1,type:Number}),fill:()=>({value:[16,27,38,49,60,71,82,93],type:Array}),stagger:()=>({value:{each:5,grid:{col:11,row:11,direction:"row"},waitComplete:!1},type:"any"}),reorder:()=>({value:!0,type:Boolean}),disableOffcanvas:!!(Re()||ne())}});var od=({canvas:e,numberOfRow:t,numberOfColumn:r,cellWidth:o,cellHeight:s,gutter:n,fill:i,disableOffcanvas:a})=>{let{useOffscreen:l,context:p}=je({disableOffcanvas:a}),u=!0,m=[],d=[],f={},T={},g={},{top:S,left:x}=ie(e),C=e.getContext(p,{alpha:!1}),_="#000",V="#fff",{activeRoute:P}=O.get(),{offscreen:I,offScreenCtx:F}=ze({useOffscreen:l,canvas:e});e.width=e.clientWidth,e.height=e.clientHeight,m=Zr({canvas:e,numberOfRow:t,numberOfColumn:r,cellWidth:o,cellHeight:s,gutter:n}).items,d=m.map((M,y)=>({...M,scale:0,mouseX:0,mouseY:0,hasFill:i.includes(y)})).sort(M=>M.hasFill?-1:1),f=Z.createLerp({data:{mouseX:0,mouseY:0}}),d.forEach(M=>{f.subscribeCache(M,({mouseX:y,mouseY:v})=>{M.mouseX=y,M.mouseY=v})}),T=Z.createTween({ease:"easeInOutSine",stagger:{each:5,from:"center",waitComplete:!1},data:{scale:0}}),d.forEach(M=>{T.subscribeCache(M,({scale:y})=>{M.scale=y})});let U=()=>{if(!C)return;l&&(I.width=e.width,I.height=e.height);let M=l?F:C;M.fillStyle="#1a1b26",M.fillRect(0,0,e.width,e.height),d.forEach(({x:y,y:v,centerX:R,centerY:A,width:W,height:Y,mouseX:ee,mouseY:Ie,scale:ae,hasFill:de,offsetXCenter:Me,offsetYCenter:De})=>{let Le=ee-(e.width-(W+n)*r)/2,Ge=Ie-(e.height-(Y+n)*t)/2,Je=(y-Le)/250,vt=(v-Ge)/250,St=Math.sqrt(Math.pow(Math.abs(Je),2)+Math.pow(Math.abs(vt),2)),Yt=Oe(Math.abs(St),0,2),eo=0,Ue=Math.cos(eo)*(Yt+ae),Lo=Math.sin(eo)*(Yt+ae);M.setTransform(Ue,Lo,-Lo,Ue,Math.round(R+Me),Math.round(A+De)),M.beginPath(),M.rect(Math.round(-R+y),Math.round(-A+v),W,Y),M.fillStyle=de?V:_,M.fill(),M.setTransform(1,0,0,1,0,0)}),He({useOffscreen:l,offscreen:I,ctx:C})};g=Ne.createAsyncTimeline({repeat:-1,yoyo:!0}).goTo(T,{scale:.3},{duration:1e3}),g.play();let j=({x:M,y})=>{f.goTo({mouseX:M-x,mouseY:y-S})},G=c.useMouseMove(({client:M})=>{let{x:y,y:v}=M;j({x:y,y:v})}),D=c.useTouchMove(({client:M})=>{let{x:y,y:v}=M;j({x:y,y:v})}),N=()=>{U(),u&&c.useNextFrame(()=>N())};c.useFrame(({time:M})=>{N({time:M})});let q=c.useResize(()=>{e.width=e.clientWidth,e.height=e.clientHeight,S=ie(e).top,x=ie(e).left,d.forEach(M=>{let{width:y,height:v,gutter:R,numberOfColumn:A}=M;M.offsetXCenter=pr({canvasWidth:e.width,width:y,gutter:R,numberOfColumn:A}),M.offsetYCenter=hr({canvasHeight:e.height,height:v,gutter:R,numberOfRow:t})}),c.useFrame(()=>U())}),Q=$.watch("openNavigation",()=>{g?.stop(),u=!1}),B=$.watch("closeNavigation",()=>setTimeout(async()=>{u=!0;let{activeRoute:M}=O.get();M===P&&(g?.play(),c.useFrame(()=>N()))},500));return()=>{T.destroy(),g.destroy(),f.destroy(),q(),G(),D(),B(),Q(),T=null,g=null,f=null,C=null,I=null,F=null,m=[],d=[],u=!1}};var sd=({onMount:e,html:t,getState:r,staticProps:o})=>{e(({refs:i})=>{if(oe.mq("max","desktop"))return;let{wrap:a,canvas:l}=i,p=od({canvas:l,...r()});return c.useFrame(()=>{a.classList.add("active")}),()=>{p()}});let{animatedPatternN1:s}=ye(),{source:n}=s;return t`
        <div>
            <only-desktop></only-desktop>
            <code-button
                ${o({drawers:[{label:"description",source:n.description},{label:"definition",source:n.definition},{label:"component",source:n.component},{label:"animation",source:n.animation}],style:"legend",color:"green"})}
    ...
    `};var Mb=k({name:"animatedpattern-n1",component:sd,isolateCreation:!0,isolateOnMount:!0,exportState:["numberOfRow","numberOfColumn","cellWidth","cellHeight","gutter","fill","disableOffcanvas"],state:{numberOfRow:7,numberOfColumn:15,cellWidth:70,cellHeight:70,gutter:10,fill:[21,22,23,24,25,25,26,37,42,53,58,69,74,85,86,87,88,89,90,44,60,65,66],disableOffcanvas:!!(Re()||ne())}});function Lb({width:e,relativeIndex:t,amountOfPath:r}){return Math.sqrt(Math.pow(e*t,2)-Math.pow(e*t/r*t,2))*2}function Fb({height:e,relativeIndex:t,amountOfPath:r}){return Math.sqrt(Math.pow(e*t,2)-Math.pow(e*t/r*t,2))*2}var nd=({canvas:e,amountOfPath:t,width:r,height:o,fill:s,stroke:n,opacity:i,spacerY:a,intialRotation:l,perpetualRatio:p,mouseMoveRatio:u,disableOffcanvas:m})=>{let{useOffscreen:d,context:f}=je({disableOffcanvas:m}),T=!0,g=e.getContext(f,{alpha:!1}),S=[],x=[],C={},{left:_}=ie(e),{activeRoute:V}=O.get(),{offscreen:P,offScreenCtx:I}=ze({useOffscreen:d,canvas:e}),F=d?I:g,U=yt(F)&&!ne();F=null,e.width=e.clientWidth,e.height=e.clientHeight,S=[...new Array(t).keys()].map((y,v)=>{let R=v,A=R<t/2?t-R:R,W=A-(t-A);return{width:Math.floor(Lb({width:r,relativeIndex:W,amountOfPath:t})),height:Math.floor(Fb({height:o,relativeIndex:W,amountOfPath:t})),fill:s,stroke:n,opacity:W*i,rotate:0,y:0,relativeIndex:W,index:v}}),x=S.splice(0,S.length/2).concat(S.reverse()),C=Z.createSpring({data:{rotate:0,y:0},stagger:{each:5,from:"center"}}),[...x].forEach(y=>{C.subscribeCache(y,({rotate:v})=>{y.rotate=v})});let j=({time:y=0})=>{if(!g)return;d&&(P.width=e.width,P.height=e.height);let v=d?I:g,R=e.width/2,A=e.height/2;v.fillStyle="#1a1b26",v.fillRect(0,0,e.width,e.height),x.forEach(({width:W,height:Y,opacity:ee,rotate:Ie,relativeIndex:ae,index:de})=>{let Me=Math.sin(y/1e3)*p*ae,De=de<t/2?Me+15*ae/2:-Me-15*ae/2,Le=de<t/2?-1:1,Ge=1,Je=Math.PI/180*(Ie-l),vt=Math.cos(Je)*Ge,St=Math.sin(Je)*Ge;v.setTransform(vt,St,-St,vt,R,A+Y/2),U?(v.beginPath(),v.roundRect(-(W*Le)/2,-Y/2+De+a(de<t/2),W,Y,[200,0])):(v.beginPath(),v.rect(-(W*Le)/2,-Y/2+De+a(de<t/2),W,Y)),v.strokeStyle=`rgba(255, 255, 255, ${ee})`,v.fillStyle=`rgba(26, 27, 38, ${ee})`,v.stroke(),v.fill(),v.setTransform(1,0,0,1,0,0)}),He({useOffscreen:d,offscreen:P,ctx:g})},G=({time:y=0})=>{j({time:y}),T&&c.useNextFrame(({time:v})=>G({time:v}))};c.useFrame(({time:y})=>{G({time:y})});let D=c.useResize(()=>{e.width=e.clientWidth,e.height=e.clientHeight,_=ie(e).left,c.useFrame(({time:y})=>{j({time:y})})}),N=({x:y})=>{let v=y-e.width/2-_;C.goTo({rotate:v/u})},q=c.useMouseMove(({client:y})=>{let{x:v}=y;N({x:v})}),Q=c.useTouchMove(({client:y})=>{let{x:v}=y;N({x:v})}),B=$.watch("openNavigation",()=>{T=!1}),M=$.watch("closeNavigation",()=>{setTimeout(()=>{T=!0;let{activeRoute:y}=O.get();y===V&&c.useFrame(({time:v})=>G({time:v}))},500)});return()=>{C.destroy(),D(),q(),Q(),M(),B(),g=null,P=null,I=null,C=null,x=[],S=[],T=!1}};var id=({onMount:e,html:t,getState:r,staticProps:o})=>{e(({refs:a})=>{if(oe.mq("max","desktop"))return;let{wrap:l,canvas:p}=a,u=nd({canvas:p,...r()});return c.useFrame(()=>{l.classList.add("active")}),()=>{u()}});let{caterpillarN0:s}=ye(),{source:n}=s,i=ne()?"c-canvas__wrap--wrapped":"";return t`
        <div>
            <only-desktop></only-desktop>
            <code-button
                ${o({drawers:[{label:"description",source:n.description},{label:"definition",source:n.definition},{label:"component",source:n.component},{label:"animation",source:n.animation}],style:"legend",color:"green"})}
    ...
    `};var Db=k({name:"caterpillar-n0",component:id,isolateOnMount:!0,isolateCreation:!0,exportState:["amountOfPath","width","height","radius","fill","stroke","opacity","spacerY","intialRotation","perpetualRatio","mouseMoveRatio","disableOffcanvas"],state:{amountOfPath:17,width:Re()||ne()?30:40,height:Re()||ne()?30:40,radius:0,fill:"",stroke:"#fff",opacity:.05,spacerY:e=>e?300:-400,intialRotation:33,perpetualRatio:6,mouseMoveRatio:10,disableOffcanvas:!!(Re()||ne())}});var ad=({canvas:e,numItems:t,width:r,height:o,fill:s,opacity:n,radius:i,rotationDuration:a,rotationEach:l,centerEach:p,disableOffcanvas:u})=>{let{useOffscreen:m,context:d}=je({disableOffcanvas:u}),f=!0,T=e.getContext(d,{alpha:!1}),g=[],S={},x={},C={},{top:_,left:V}=ie(e),{activeRoute:P}=O.get(),{offscreen:I,offScreenCtx:F}=ze({useOffscreen:m,canvas:e}),U=m?F:T,j=yt(U)&&!ne();U=null,e.width=e.clientWidth,e.height=e.clientHeight,g=[...new Array(t).keys()].map((v,R)=>{let A=R>=t/2?t/2+(t/2-R):R,W=s.includes(R)?1:A*n;return{width:A*r,height:A*o,x:0,y:0,hasFill:s.includes(R),opacity:W,radius:i,rotate:0,relativeIndex:A}}),S=Z.createTween({data:{rotate:0},stagger:{each:l,from:"center"},ease:"easeLinear",relative:!0}),[...g].forEach(v=>{S.subscribeCache(v,({rotate:R})=>{v.rotate=R})}),x=Z.createSpring({data:{x:0,y:0},stagger:{each:p,from:"end"}}),[...g].forEach(v=>{x.subscribeCache(v,({x:R,y:A})=>{v.x=R,v.y=A})});let G=()=>{if(!T)return;m&&(I.width=e.width,I.height=e.height);let v=m?F:T;v.fillStyle="#1a1b26",v.fillRect(0,0,e.width,e.height),g.forEach(({width:R,height:A,x:W,y:Y,opacity:ee,rotate:Ie,hasFill:ae},de)=>{let Me=g.length-de,De=e.width/2,Le=e.height/2,Ge=1,Je=Math.PI/180*Ie,vt=Math.cos(Je)*Ge,St=Math.sin(Je)*Ge;v.setTransform(vt,St,-St,vt,De+W+Me*W/20,Le+Y+Me*Y/20),j?(v.beginPath(),v.roundRect(Number.parseInt(-R/2),Number.parseInt(-A/2),R,A,[200,0])):(v.beginPath(),v.rect(Number.parseInt(-R/2),Number.parseInt(-A/2),R,A)),ae?v.fillStyle="rgba(255, 255, 255, 1)":(v.fillStyle=`rgba(26, 27, 38, ${ee})`,v.strokeStyle=`rgba(255, 255, 255, ${ee})`,v.stroke()),v.fill(),v.setTransform(1,0,0,1,0,0)}),He({useOffscreen:m,offscreen:I,ctx:T})};C=Ne.createAsyncTimeline({repeat:-1,yoyo:!1}),C.goTo(S,{rotate:360},{duration:a}),C.play();let D=()=>{G(),f&&c.useNextFrame(()=>D())};c.useFrame(()=>D());let N=c.useResize(()=>{e.width=e.clientWidth,e.height=e.clientHeight,_=ie(e).top,V=ie(e).left,G()}),q=({x:v,y:R})=>{let A=window.innerWidth,W=window.innerHeight,Y=v-e.width/2-V,ee=R-e.height/2-_;x.goTo({x:Oe(Y,-A/2+400+V,A/2-400-V),y:Oe(ee,-W/2+200+_,W/2-200-_)})},Q=c.useMouseMove(({client:v})=>{let{x:R,y:A}=v;q({x:R,y:A})}),B=c.useTouchMove(({client:v})=>{let{x:R,y:A}=v;q({x:R,y:A})}),M=$.watch("openNavigation",()=>{f=!1,C?.pause()}),y=$.watch("closeNavigation",()=>setTimeout(()=>{f=!0;let{activeRoute:v}=O.get();v===P&&(C?.resume(),c.useFrame(()=>D()))},500));return()=>{S.destroy(),x.destroy(),C.destroy(),N(),Q(),B(),M(),y(),S=null,x=null,C=null,T=null,I=null,F=null,g=[],f=!1}};var ld=({onMount:e,html:t,getState:r,staticProps:o})=>{e(({refs:a})=>{if(oe.mq("max","desktop"))return;let{wrap:l,canvas:p}=a,u=ad({canvas:p,...r()});return c.useFrame(()=>{l.classList.add("active")}),()=>{u()}});let{caterpillarN1:s}=ye(),{source:n}=s,i=ne()?"c-canvas__wrap--wrapped":"";return t`
        <div>
            <only-desktop></only-desktop>
            <code-button
                ${o({drawers:[{label:"description",source:n.description},{label:"definition",source:n.definition},{label:"component",source:n.component},{label:"animation",source:n.animation}],style:"legend",color:"green"})}
    ...
    `};var Vb=k({name:"caterpillar-n1",component:ld,isolateCreation:!0,isolateOnMount:!0,exportState:["numItems","width","height","fill","opacity","radius","rotationEach","centerEach","rotationDuration","disableOffcanvas"],state:{numItems:20,width:40,height:40,fill:[14],opacity:.05,radius:0,rotationEach:15,centerEach:3,rotationDuration:5e3,disableOffcanvas:!!(Re()||ne())}});var Ei=({value:e,direction:t,isForced:r})=>{r||console.log(`current: ${e}, direction: ${t}`)},cd=({canvas:e,numItems:t,width:r,height:o,radius:s,fill:n,opacity:i,xAmplitude:a,yAmplitude:l,duration:p,friction:u,rotationDefault:m,disableOffcanvas:d})=>{let{useOffscreen:f,context:T}=je({disableOffcanvas:d}),g=!0,S=e.getContext(T,{alpha:!1}),x=[],C=m,{activeRoute:_}=O.get(),{offscreen:V,offScreenCtx:P}=ze({useOffscreen:f,canvas:e}),I=f?P:S,F=yt(I)&&!ne();I=null,x=[...new Array(t).keys()].map((B,M)=>{let y=M>=t/2?t/2+(t/2-M):M,v=r+r/3*y,R=o+o/3*y,A=n.includes(M)?1:(t-M)*i;return{width:v,height:R,x:0,y:0,hasFill:n.includes(M),opacity:A,radius:s,rotate:0}}),e.width=e.clientWidth,e.height=e.clientHeight;let U=Z.createSequencer({stagger:{each:6},data:{x:p/4,rotate:0},duration:p}).goTo({x:p+p/4},{start:0,end:p,ease:"easeLinear"}).goTo({rotate:()=>-C},{start:0,end:5,ease:"easeInOutBack"}).goTo({rotate:0},{start:5,end:p,ease:"easeInOutBack"}).label("mylabel",2).add(({isForced:B,direction:M})=>{Ei({isForced:B,direction:M,value:1})},1).add(({isForced:B,direction:M})=>{Ei({isForced:B,direction:M,value:5})},5).add(({isForced:B,direction:M})=>{Ei({isForced:B,direction:M,value:9})},9);x.forEach(B=>{U.subscribeCache(B,({x:M,rotate:y})=>{let v=M/u,R=2/(3-Math.cos(2*v)),A=R*Math.cos(v)*a,W=R*Math.sin(2*v)/2*l;B.x=A,B.y=W,B.rotate=y})});let j=Ne.createSyncTimeline({repeat:-1,yoyo:!1,duration:4e3}).add(U);j.onLoopEnd(({loop:B,direction:M})=>{console.log(`loop end: ${B} , ${M}`)});let G=()=>{if(!S)return;f&&(V.width=e.width,V.height=e.height);let B=f?P:S;B.fillStyle="#1a1b26",B.fillRect(0,0,e.width,e.height),x.forEach(({width:M,height:y,x:v,y:R,radius:A,rotate:W,hasFill:Y,opacity:ee})=>{let Ie=e.width/2,ae=e.height/2,de=1,Me=Math.PI/180*W,De=Math.cos(Me)*de,Le=Math.sin(Me)*de;B.setTransform(De,Le,-Le,De,Ie+v,ae+R),F?(B.beginPath(),B.roundRect(Number.parseInt(-M/2),Number.parseInt(-y/2),M,y,[150,0])):(B.beginPath(),B.rect(Number.parseInt(-M/2),Number.parseInt(-y/2),M,y,A)),Y?B.fillStyle="rgba(255, 255, 255, 1)":(B.fillStyle=`rgba(26, 27, 38, ${ee})`,B.strokeStyle=`rgba(255, 255, 255, ${ee})`,B.stroke()),B.fill(),B.setTransform(1,0,0,1,0,0)}),He({useOffscreen:f,offscreen:V,ctx:S})},D=()=>{G(),g&&c.useNextFrame(()=>D())};c.useFrame(()=>D()),j.play();let N=c.useResize(()=>{e.width=e.clientWidth,e.height=e.clientHeight,G()}),q=$.watch("openNavigation",()=>{g=!1,j?.pause()}),Q=$.watch("closeNavigation",()=>setTimeout(()=>{g=!0;let{activeRoute:B}=O.get();B===_&&(j?.resume(),c.useFrame(()=>D()))},500));return{destroy:()=>{g=!1,N(),q(),Q(),U.destroy(),j.destroy(),S=null,V=null,P=null,x=[]},play:()=>{j.stop(),j.play()},playReverse:()=>{j.stop(),j.playReverse()},playUseCurrent:()=>j.play({useCurrent:!0}),playReverseUseCurrent:()=>j.playReverse({useCurrent:!0}),playFromLabel:()=>{j.stop(),j.playFrom("mylabel")},plaFromLabelReverse:()=>{j.stop(),j.playFromReverse("mylabel")},stop:()=>j.stop(),pause:()=>j.pause(),resume:()=>j.resume(),reverse:()=>j.reverse(),setRotation:B=>C=B}};function $b({buttons:e}){return Object.entries(e).map(([t,r])=>{let{label:o}=r;return b` <li class="c-canvas__controls__item">
                <button
                    type="button"
            ...
            </li>`}).join("")}var ud=({onMount:e,html:t,getState:r,staticProps:o})=>{let{buttons:s,rotationDefault:n}=r();e(({element:p,refs:u})=>{if(oe.mq("max","desktop"))return;let{wrap:m,canvas:d,rangeValue:f,rotationButton:T}=u,g=cd({canvas:d,...r()}),{destroy:S,setRotation:x}=g;return Object.entries(s).forEach(([C,_])=>{let{method:V}=_;p.querySelector(`.${C}`).addEventListener("click",()=>g?.[V]())}),T.addEventListener("change",()=>{let C=T.value;x(C),f.textContent=C}),c.useFrame(()=>{m.classList.add("active")}),()=>{S()}});let{caterpillarN2:i}=ye(),{source:a}=i,l=ne()?"c-canvas__wrap--wrapped":"";return t`
        <div>
            <only-desktop></only-desktop>
            <code-button
...
`;var hd=({logoRefs:e,around:t})=>{let r=e.map(p=>{let[u,m]=Object.entries(p)[0];return{key:u,item:m}}),o=Z.createTween({data:{opacity:0,scale:.5,x:-10},duration:2e3,ease:"easeOutQuart",stagger:{each:8,from:"end"}}),s=Z.createTween({data:{scale:1,x:0},duration:4e3,ease:"easeInOutQuad",stagger:{each:40,from:"end"}}),n=Z.createTween({data:{scale:.5,opacity:0},duration:1e3,ease:"easeInOutQuad",stagger:{each:4}}),i=Z.createTween({data:{scale:1},duration:4e3,ease:"easeInOutQuad",stagger:{each:20}});r.forEach(({key:p,item:u})=>{s.subscribe(({scale:m,x:d})=>{if(u.style.scale=`${m}`,p!=="M_right"&&p!=="M_left")return;let f=p==="M_right"?-d:d;u.style.translate=`${f}px 0px`})}),r.forEach(({key:p,item:u})=>{o.subscribe(({scale:m,x:d,opacity:f})=>{if(u.style.scale=`${m}`,u.style.opacity=f,p!=="M_right"&&p!=="M_left")return;let T=p==="M_right"?-d:d;u.style.translate=`${T}px 0px`})}),t.forEach(p=>{n.subscribe(({scale:u,opacity:m})=>{p.style.scale=`${u}`,p.style.opacity=m})}),t.forEach(p=>{i.subscribe(({scale:u})=>{p.style.scale=`${u}`})});let a=Ne.createAsyncTimeline({repeat:1}).createGroup({waitComplete:!0}).goTo(o,{opacity:1,scale:1,x:0}).goTo(n,{opacity:1,scale:1}).closeGroup(),l=Ne.createAsyncTimeline({repeat:-1,yoyo:!0}).createGroup({waitComplete:!1}).goTo(s,{scale:.95,x:.5}).goTo(i,{scale:.95}).closeGroup().createGroup({waitComplete:!1}).goTo(s,{scale:1.05,x:-.5}).goTo(i,{scale:1.05}).closeGroup();return{playIntro:async()=>a.play(),playSvg:()=>l.play(),destroySvg:()=>{l.stop(),s.destroy(),l.destroy(),a.destroy(),n.destroy(),s=null,o=null,i=null,n=null,l=null,a=null}}};var dd=({refs:e})=>{let t=Z.createTween({data:{y:100},duration:500,ease:"easeOutCubic",stagger:{each:10}});return e.forEach(r=>{t.subscribe(({y:o})=>{r.style.translate=`0px ${o}%`})}),{playText:()=>t.goTo({y:0}),destroyText:()=>{t.destroy(),t=null}}};var Ub=async({playText:e,playIntro:t,playSvg:r})=>{e(),await t(),r()},md=({html:e,onMount:t,staticProps:r})=>{t(({element:n,refs:i})=>{let{textStagger:a,block1:l,block2:p,block3:u,block4:m,block5:d,block6:f,block7:T,block8:g,M_left:S,M_right:x,around:C}=i,{playIntro:_,playSvg:V,destroySvg:P}=hd({element:n,logoRefs:[{block1:l},{block2:p},{block3:u},{block4:m},{block5:d},{block6:f},{block7:T},{block8:g},{M_left:S},{M_right:x}],around:C}),{playText:I,destroyText:F}=dd({refs:a});return Ub({playText:I,playIntro:_,playSvg:V}),()=>{P(),F()}});let{home:o}=ye(),{source:s}=o;return e`<div>
        ...
        <div class="l-index__top-right">${ki}</div>
            ...
            `).join("")},Kb=({numOfCol:e,setState:t,bindProps:r,staticProps:o,delegateEvents:s})=>[...new Array(e).keys()].map((n,i)=>b`
                ...
                ></horizontal-scroller-button>
            `).join(""),yd=({onMount:e,html:t,getState:r,setState:o,watch:s,staticProps:n,bindProps:i,delegateEvents:a})=>{let{animatePin:l}=r();e(({element:m})=>{if(oe.mq("max","desktop"))return;let d=m.querySelectorAll(".js-indicator"),f=m.querySelector(".js-nav"),T=m.querySelectorAll(".js-title h1"),{destroy:g}=bd({indicators:d,titles:T,nav:f,...r(),setState:o});return window.scrollTo(0,0),s("currentId",S=>{if(S===-1)return;let x=m.querySelector(`.shadowClass--section-${S} .shadowClass--in-center`),{top:C}=ie(x),_=Se(x),V=Number.parseInt(S)===0?window.innerHeight+1:C+_-window.innerHeight;At.to(V,{duration:2e3})}),()=>{g()}});let{horizontalScroller:p}=ye(),{source:u}=p;return oe.mq("max","desktop")?t`<div><only-desktop></only-desktop></div>`:t`<div class="l-h-scroller">
        <only-desktop></only-desktop>
...
`;var wd=({canvas:e,canvasScroller:t,numberOfRow:r,numberOfColumn:o,cellWidth:s,cellHeight:n,gutter:i,fill:a,stagger:l,reorder:p,disableOffcanvas:u})=>{let{useOffscreen:m,context:d}=je({disableOffcanvas:u}),f=!0,T=[],g=[],S=Z.createMasterSequencer(),x=e.getContext(d,{alpha:!1}),C="#000",_="#fff",{activeRoute:V}=O.get(),{offscreen:P,offScreenCtx:I}=ze({useOffscreen:m,canvas:e}),F=m?I:x,U=yt(F);F=null,e.width=e.clientWidth,e.height=e.clientHeight,T=Zr({canvas:e,numberOfRow:r,numberOfColumn:o,cellWidth:s,cellHeight:n,gutter:i}).items,g=p?T.map((y,v)=>({...y,scale:0,rotate:0,hasFill:a.includes(v)})).sort(y=>y.hasFill?-1:1):T.map((y,v)=>({...y,scale:0,rotate:0,hasFill:a.includes(v)}));let j=Z.createStaggers({items:g,stagger:l}),G=j.map(({item:y,start:v,end:R})=>{let A=y.hasFill?1.1:1,W=Z.createSequencer({data:{scale:0}}).goTo({scale:A},{start:v,end:R,ease:"easeOutBack"}),Y=W.subscribe(({scale:ee})=>{y.scale=ee});return S.add(W),{sequencer:W,unsubscribe:Y}}),D=()=>{if(!x)return;m&&(P.width=e.width,P.height=e.height);let y=m?I:x;y.fillStyle="#1a1b26",y.fillRect(0,0,e.width,e.height),g.forEach(({x:v,y:R,centerX:A,centerY:W,width:Y,height:ee,rotate:Ie,scale:ae,hasFill:de,offsetXCenter:Me,offsetYCenter:De})=>{let Le=Math.PI/180*Ie,Ge=Math.cos(Le)*ae,Je=Math.sin(Le)*ae;y.setTransform(Ge,Je,-Je,Ge,Math.round(A+Me),Math.round(W+De)),ed(y,Math.round(-A+v),Math.round(-W+R),Y,ee,5),U?(y.beginPath(),y.roundRect(Math.round(-A+v),Math.round(-W+R),Y,ee,5)):(y.beginPath(),y.rect(Math.round(-A+v),Math.round(-W+R),Y,ee)),y.fillStyle=de?_:C,y.fill(),y.setTransform(1,0,0,1,0,0)}),He({useOffscreen:m,offscreen:P,ctx:x})},N=Gt.createScrollTrigger({trigger:t,propierties:"tween",tween:S,dynamicStart:{position:"bottom",value:()=>window.innerHeight},dynamicEnd:{position:"bottom",value:()=>Se(t)},ease:!0,easeType:"lerp"});N.init();let q=()=>{D(),f&&c.useNextFrame(()=>q())};c.useFrame(({time:y})=>{q({time:y})});let Q=c.useResize(()=>{e.width=e.clientWidth,e.height=e.clientHeight,g.forEach(y=>{let{width:v,height:R,gutter:A,numberOfColumn:W}=y;y.offsetXCenter=pr({canvasWidth:e.width,width:v,gutter:A,numberOfColumn:W}),y.offsetYCenter=hr({canvasHeight:e.height,height:R,gutter:A,numberOfRow:r})}),c.useFrame(()=>D())}),B=$.watch("openNavigation",()=>{f=!1}),M=$.watch("closeNavigation",()=>setTimeout(async()=>{f=!0;let{activeRoute:y}=O.get();y===V&&c.useFrame(()=>q())},500));return()=>{Q(),M(),B(),G.forEach(({sequencer:y,unsubscribe:v})=>{y.destroy(),v()}),G=[],S.destroy(),S=null,j=[],N.destroy(),N=null,x=null,P=null,I=null,T=[],g=[],f=!1}};var Td=({onMount:e,html:t,getState:r,staticProps:o})=>{e(({refs:i})=>{if(oe.mq("max","desktop"))return;let{wrap:a,canvas:l,canvasScroller:p}=i;window.scrollTo(0,0);let u=wd({canvas:l,canvasScroller:p,...r()});return c.useFrame(()=>{a.classList.add("active")}),()=>{u()}});let{scrollerN0:s}=ye(),{source:n}=s;return oe.mq("max","desktop")?t`<div><only-desktop></only-desktop></div>`:t`
        <div>
            <only-desktop></only-desktop>
    ...
    `};var r0=k({name:"scroller-n0",component:Td,isolateOnMount:!0,isolateCreation:!0,exportState:["numberOfRow","numberOfColumn","cellWidth","cellHeight","gutter","fill","stagger","reorder","disableOffcanvas"],state:{numberOfRow:()=>({value:10,type:Number}),numberOfColumn:()=>({value:10,type:Number}),cellWidth:()=>({value:65,type:Number}),cellHeight:()=>({value:65,type:Number}),gutter:()=>({value:1,type:Number}),fill:()=>({value:[36,37,38,39,40,47,51,58,62,69,73,80,81,82,83,84],type:Array}),stagger:()=>({value:{type:"equal",each:6,from:"random"},type:"Any"}),reorder:()=>({value:!0,type:Boolean}),disableOffcanvas:!!(Re()||ne())}});function o0({width:e,relativeIndex:t,amountOfPath:r}){return Math.sqrt(Math.pow(e*t,2)-Math.pow(e*t/r*t,2))*2}function s0({height:e,relativeIndex:t,amountOfPath:r}){return Math.sqrt(Math.pow(e*t,2)-Math.pow(e*t/r*t,2))*2}var xd=({canvas:e,canvasScroller:t,amountOfPath:r,width:o,height:s,radius:n,opacity:i,intialRotation:a,endRotation:l,disableOffcanvas:p})=>{let{useOffscreen:u,context:m}=je({disableOffcanvas:p}),d=!0,f=e.getContext(m,{alpha:!1}),T=[],{activeRoute:g}=O.get(),{offscreen:S,offScreenCtx:x}=ze({useOffscreen:u,canvas:e}),C=u?x:f,_=yt(C)&&!ne();C=null,e.width=e.clientWidth,e.height=e.clientHeight,T=[...new Array(r).keys()].map((D,N)=>{let q=N>=r/2?r/2+(r/2-N):N;return{width:Math.floor(o0({width:o,relativeIndex:q,amountOfPath:r})),height:Math.floor(s0({height:s,relativeIndex:q,amountOfPath:r})),opacity:q*i,rotate:0,relativeIndex:q,index:N}});let V=Z.createScrollerTween({from:{rotate:0},to:{rotate:l},stagger:{each:5,from:"center"}});[...T].forEach(D=>{V.subscribeCache(D,({rotate:N})=>{D.rotate=N})});let P=()=>{if(!f)return;u&&(S.width=e.width,S.height=e.height);let D=u?x:f,N=e.width/2,q=e.height/2;D.fillStyle="#1a1b26",D.fillRect(0,0,e.width,e.height),T.forEach(({width:Q,height:B,opacity:M,rotate:y,index:v})=>{let R=T.length/2-v,A=1,W=Math.PI/180*(y-a),Y=Math.cos(W)*A,ee=Math.sin(W)*A;D.setTransform(Y,ee,-ee,Y,N,q+R*19),_?(D.beginPath(),D.roundRect(-Q/2,-B/2+R*19,Q,B,150)):(D.beginPath(),D.rect(Number.parseInt(-Q/2),Number.parseInt(-B/2),Q,B,n)),D.strokeStyle=`rgba(255, 255, 255, ${M})`,D.fillStyle=`rgba(26, 27, 38, ${M})`,D.stroke(),D.fill(),D.setTransform(1,0,0,1,0,0)}),He({useOffscreen:u,offscreen:S,ctx:f})},I=Gt.createScrollTrigger({trigger:t,propierties:"tween",tween:V,dynamicStart:{position:"bottom",value:()=>window.innerHeight},dynamicEnd:{position:"bottom",value:()=>Se(t)},ease:!0,easeType:"spring"});I.init();let F=({time:D=0})=>{P({time:D}),d&&c.useNextFrame(({time:N})=>F({time:N}))};c.useFrame(({time:D})=>{F({time:D})});let U=c.useResize(()=>{e.width=e.clientWidth,e.height=e.clientHeight,c.useFrame(()=>{P()})}),j=$.watch("openNavigation",()=>{d=!1}),G=$.watch("closeNavigation",()=>{setTimeout(()=>{d=!0;let{activeRoute:D}=O.get();D===g&&c.useFrame(({time:N})=>F({time:N}))},500)});return()=>{V.destroy(),U(),G(),j(),V.destroy(),V=null,I.destroy(),I=null,f=null,S=null,x=null,V=null,T=[],d=!1}};var Cd=({onMount:e,html:t,getState:r,staticProps:o})=>{e(({refs:a})=>{if(oe.mq("max","desktop"))return;let{wrap:l,canvas:p,canvasScroller:u}=a,m=xd({canvas:p,canvasScroller:u,...r()});return c.useFrame(()=>{l.classList.add("active")}),()=>{m()}});let{scrollerN1:s}=ye(),{source:n}=s,i=ne()?"c-canvas__wrap--wrapped":"";return oe.mq("max","desktop")?t`<div><only-desktop></only-desktop></div>`:t`
        <div>
            <only-desktop></only-desktop>
            <div class="c-canvas c-canvas--fixed ">
                <code-button
    ...
    </div>`};var S0=k({name:"dynamic-slotted-label",component:Dd,exportState:["label"],state:{label:()=>({value:void 0,type:"Any"})}});var Vd=({groups:e,trails:t,boxWidth:r,boxHeight:o,svg:s})=>{let n=180/Math.PI,i=window.innerWidth,a=window.innerHeight,l=s.clientWidth,p=s.clientHeight,u=0,m=0,d=0,f=0,T=t.map(P=>P.querySelector("path")),g=Z.createSpring({data:{x:0,y:0},stagger:{each:2,from:"start"}});t.forEach(P=>{g.subscribe(({x:I,y:F})=>{P.style.transform=`translate(${I}px, ${F}px)`})});let S=Z.createSpring({data:{rotation:0},stagger:{each:5,from:"start"}});T.forEach(P=>{S.subscribeCache(P,({rotation:I})=>{P.style.rotate=`${I}deg`})});let x=c.useResize(()=>{i=window.innerWidth,a=window.innerHeight,l=s.clientWidth,p=s.clientHeight}),C=c.useMouseMove(({client:P})=>{let{x:I,y:F}=P,G=(I*r/i-r/2)*i/l,q=(F*o/a-o/2)*a/p,Q=q-u,B=G-m;if(Math.abs(B)>10||Math.abs(Q)>10){u=q,m=G;let y=Math.atan2(Q,B)*n+180,v=Math.abs(d-y);v>180&&d<y&&(f-=360),v>180&&d>y&&(f+=360);let R=y+f+90;S.goTo({rotation:R}),d=y}g.goTo({x:G,y:q})}),_=Z.createTween({data:{opacity:0,scale:.95},duration:2e3,ease:"easeOutQuart",stagger:{each:5,from:"center"}});e.forEach(P=>{_.subscribeCache(P,({scale:I,opacity:F})=>{P.style.scale=`${I}`,P.style.opacity=F})});let V=Ne.createAsyncTimeline({repeat:1}).goTo(_,{opacity:1,scale:1});return{playIntro:async()=>V.play(),destroy:()=>{_.destroy(),_=null,V.destroy(),V=null,g.destroy(),g=null,S.destroy(),S=null,x(),C(),i=null,a=null,l=null,p=null,u=null,m=null,d=null,f=null,T=null}}};var w0=async({playIntro:e})=>{await e()},$d=({onMount:e,html:t,getState:r,staticProps:o})=>{let s=oe.mq("min","desktop"),{svg:n}=s?r():"",{child:i}=ye(),{source:a}=i;return e(({element:l,refs:p})=>{if(!s)return;let u=l.querySelector("svg"),{width:m,height:d}=u.viewBox.baseVal,{trail1:f,trail2:T,trail3:g,trail4:S,black:x,body:C,bottom_green:_,collo:V,dark_shadow:P,gambe:I,green_top:F,head:U,light_shadow:j,head_bg:G,body_bg:D,head_green:N}=p,q=Vd({groups:[x,C,_,V,P,I,F,U,j,G,D,N],trails:[f,T,g,S],boxWidth:m,boxHeight:d,svg:u}),{playIntro:Q,destroy:B}=q;return w0({playIntro:Q}),()=>{B()}}),t`<div class="svg-child-container">
        <only-desktop></only-desktop>
