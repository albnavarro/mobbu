...
`;var Qh=({html:e,onMount:t})=>(t(({refs:r})=>{let{shape:o}=r;c.useFrame(()=>{o.classList.add("active")})}),e`
    ...
    `};var $b=k({name:"caterpillar-n1",component:ld,isolateCreation:!0,isolateOnMount:!0,exportState:["numItems","width","height","fill","opacity","radius","rotationEach","centerEach","rotationDuration","disableOffcanvas"],state:{numItems:20,width:40,height:40,fill:[14],opacity:.05,radius:0,rotationEach:15,centerEach:3,rotationDuration:5e3,disableOffcanvas:!!(Re()||ne())}});var Ei=({value:e,direction:t,isForced:r})=>{r||console.log(`current: ${e}, direction: ${t}`)},cd=({canvas:e,numItems:t,width:r,height:o,radius:s,fill:n,opacity:i,xAmplitude:a,yAmplitude:l,duration:p,friction:u,rotationDefault:m,disableOffcanvas:d})=>{let{useOffscreen:f,context:T}=je({disableOffcanvas:d}),b=!0,w=e.getContext(T,{alpha:!1}),x=[],C=m,{activeRoute:P}=O.get(),{offscreen:M,offScreenCtx:_}=ze({useOffscreen:f,canvas:e}),E=f?_:w,V=yt(E)&&!ne();E=null,x=[...new Array(t).keys()].map((j,N)=>{let y=N>=t/2?t/2+(t/2-N):N,v=r+r/3*y,R=o+o/3*y,A=n.includes(N)?1:(t-N)*i;return{width:v,height:R,x:0,y:0,hasFill:n.includes(N),opacity:A,radius:s,rotate:0}}),e.width=e.clientWidth,e.height=e.clientHeight;let U=Z.createSequencer({stagger:{each:6},data:{x:p/4,rotate:0},duration:p}).goTo({x:p+p/4},{start:0,end:p,ease:"easeLinear"}).goTo({rotate:()=>-C},{start:0,end:5,ease:"easeInOutBack"}).goTo({rotate:0},{start:5,end:p,ease:"easeInOutBack"}).label("mylabel",2).add(({isForced:j,direction:N})=>{Ei({isForced:j,direction:N,value:1})},1).add(({isForced:j,direction:N})=>{Ei({isForced:j,direction:N,value:5})},5).add(({isForced:j,direction:N})=>{Ei({isForced:j,direction:N,value:9})},9);x.forEach(j=>{U.subscribeCache(j,({x:N,rotate:y})=>{let v=N/u,R=2/(3-Math.cos(2*v)),A=R*Math.cos(v)*a,B=R*Math.sin(2*v)/2*l;j.x=A,j.y=B,j.rotate=y})});let W=Ne.createSyncTimeline({repeat:-1,yoyo:!1,duration:4e3}).add(U);W.onLoopEnd(({loop:j,direction:N})=>{console.log(`loop end: ${j} , ${N}`)});let Y=()=>{if(!w)return;f&&(M.width=e.width,M.height=e.height);let j=f?_:w;j.fillStyle="#1a1b26",j.fillRect(0,0,e.width,e.height),x.forEach(({width:N,height:y,x:v,y:R,radius:A,rotate:B,hasFill:J,opacity:Q})=>{let Ie=e.width/2,ae=e.height/2,de=1,Me=Math.PI/180*B,De=Math.cos(Me)*de,Le=Math.sin(Me)*de;j.setTransform(De,Le,-Le,De,Ie+v,ae+R),V?(j.beginPath(),j.roundRect(Number.parseInt(-N/2),Number.parseInt(-y/2),N,y,[150,0])):(j.beginPath(),j.rect(Number.parseInt(-N/2),Number.parseInt(-y/2),N,y,A)),J?j.fillStyle="rgba(255, 255, 255, 1)":(j.fillStyle=`rgba(26, 27, 38, ${Q})`,j.strokeStyle=`rgba(255, 255, 255, ${Q})`,j.stroke()),j.fill(),j.setTransform(1,0,0,1,0,0)}),He({useOffscreen:f,offscreen:M,ctx:w})},D=()=>{Y(),b&&c.useNextFrame(()=>D())};c.useFrame(()=>D()),W.play();let L=c.useResize(()=>{e.width=e.clientWidth,e.height=e.clientHeight,Y()}),G=$.watch("openNavigation",()=>{b=!1,W?.pause()}),se=$.watch("closeNavigation",()=>setTimeout(()=>{b=!0;let{activeRoute:j}=O.get();j===P&&(W?.resume(),c.useFrame(()=>D()))},500));return{destroy:()=>{b=!1,L(),G(),se(),U.destroy(),W.destroy(),w=null,M=null,_=null,x=[]},play:()=>{W.stop(),W.play()},playReverse:()=>{W.stop(),W.playReverse()},playUseCurrent:()=>W.play({useCurrent:!0}),playReverseUseCurrent:()=>W.playReverse({useCurrent:!0}),playFromLabel:()=>{W.stop(),W.playFrom("mylabel")},plaFromLabelReverse:()=>{W.stop(),W.playFromReverse("mylabel")},stop:()=>W.stop(),pause:()=>W.pause(),resume:()=>W.resume(),reverse:()=>W.reverse(),setRotation:j=>C=j}};function Wb({buttons:e}){return Object.entries(e).map(([t,r])=>{let{label:o}=r;return g` <li class="c-canvas__controls__item">
            ...
            </li>`}).join("")}var ud=({onMount:e,html:t,getState:r,staticProps:o})=>{let{buttons:s,rotationDefault:n}=r();e(({element:p,refs:u})=>{if(re.mq("max","desktop"))return;let{wrap:m,canvas:d,rangeValue:f,rotationButton:T}=u,b=cd({canvas:d,...r()}),{destroy:w,setRotation:x}=b;return Object.entries(s).forEach(([C,P])=>{let{method:M}=P;p.querySelector(`.${C}`).addEventListener("click",()=>b?.[M]())}),T.addEventListener("change",()=>{let C=T.value;x(C),f.textContent=C}),c.useFrame(()=>{m.classList.add("active")}),()=>{w()}});let{caterpillarN2:i}=ye(),{source:a}=i,l=ne()?"c-canvas__wrap--wrapped":"";return t`
        <div>
            <only-desktop></only-desktop>
            <code-button
...
`;var hd=({logoRefs:e,around:t})=>{let r=e.map(p=>{let[u,m]=Object.entries(p)[0];return{key:u,item:m}}),o=Z.createTween({data:{opacity:0,scale:.5,x:-10},duration:2e3,ease:"easeOutQuart",stagger:{each:8,from:"end"}}),s=Z.createTween({data:{scale:1,x:0},duration:4e3,ease:"easeInOutQuad",stagger:{each:40,from:"end"}}),n=Z.createTween({data:{scale:.5,opacity:0},duration:1e3,ease:"easeInOutQuad",stagger:{each:4}}),i=Z.createTween({data:{scale:1},duration:4e3,ease:"easeInOutQuad",stagger:{each:20}});r.forEach(({key:p,item:u})=>{s.subscribe(({scale:m,x:d})=>{if(u.style.scale=`${m}`,p!=="M_right"&&p!=="M_left")return;let f=p==="M_right"?-d:d;u.style.translate=`${f}px 0px`})}),r.forEach(({key:p,item:u})=>{o.subscribe(({scale:m,x:d,opacity:f})=>{if(u.style.scale=`${m}`,u.style.opacity=f,p!=="M_right"&&p!=="M_left")return;let T=p==="M_right"?-d:d;u.style.translate=`${T}px 0px`})}),t.forEach(p=>{n.subscribe(({scale:u,opacity:m})=>{p.style.scale=`${u}`,p.style.opacity=m})}),t.forEach(p=>{i.subscribe(({scale:u})=>{p.style.scale=`${u}`})});let a=Ne.createAsyncTimeline({repeat:1}).createGroup({waitComplete:!0}).goTo(o,{opacity:1,scale:1,x:0}).goTo(n,{opacity:1,scale:1}).closeGroup(),l=Ne.createAsyncTimeline({repeat:-1,yoyo:!0}).createGroup({waitComplete:!1}).goTo(s,{scale:.95,x:.5}).goTo(i,{scale:.95}).closeGroup().createGroup({waitComplete:!1}).goTo(s,{scale:1.05,x:-.5}).goTo(i,{scale:1.05}).closeGroup();return{playIntro:async()=>a.play(),playSvg:()=>l.play(),destroySvg:()=>{l.stop(),s.destroy(),l.destroy(),a.destroy(),n.destroy(),s=null,o=null,i=null,n=null,l=null,a=null}}};var dd=({refs:e})=>{let t=Z.createTween({data:{y:100},duration:500,ease:"easeOutCubic",stagger:{each:10}});return e.forEach(r=>{t.subscribe(({y:o})=>{r.style.translate=`0px ${o}%`})}),{playText:()=>t.goTo({y:0}),destroyText:()=>{t.destroy(),t=null}}};var qb=async({playText:e,playIntro:t,playSvg:r})=>{e(),await t(),r()},md=({html:e,onMount:t,staticProps:r})=>{t(({element:n,refs:i})=>{let{textStagger:a,block1:l,block2:p,block3:u,block4:m,block5:d,block6:f,block7:T,block8:b,M_left:w,M_right:x,around:C}=i,{playIntro:P,playSvg:M,destroySvg:_}=hd({element:n,logoRefs:[{block1:l},{block2:p},{block3:u},{block4:m},{block5:d},{block6:f},{block7:T},{block8:b},{M_left:w},{M_right:x}],around:C}),{playText:E,destroyText:V}=dd({refs:a});return qb({playText:E,playIntro:P,playSvg:M}),()=>{_(),V()}});let{home:o}=ye(),{source:s}=o;return e`<div>
        ...
        <div class="l-index__top-right">${ki}</div>
            ...
            `).join("")},Zb=({numOfCol:e,setState:t,bindProps:r,staticProps:o,delegateEvents:s})=>[...new Array(e).keys()].map((n,i)=>g`
                ...
                ></horizontal-scroller-button>
            `).join(""),yd=({onMount:e,html:t,getState:r,setState:o,watch:s,staticProps:n,bindProps:i,delegateEvents:a})=>{let{animatePin:l}=r();e(({element:m})=>{if(re.mq("max","desktop"))return;let d=m.querySelectorAll(".js-indicator"),f=m.querySelector(".js-nav"),T=m.querySelectorAll(".js-title h1"),{destroy:b}=bd({indicators:d,titles:T,nav:f,...r(),setState:o});return window.scrollTo(0,0),s("currentId",w=>{if(w===-1)return;let x=m.querySelector(`.shadowClass--section-${w} .shadowClass--in-center`),{top:C}=ie(x),P=we(x),M=Number.parseInt(w)===0?window.innerHeight+1:C+P-window.innerHeight;At.to(M,{duration:2e3})}),()=>{b()}});let{horizontalScroller:p}=ye(),{source:u}=p;return re.mq("max","desktop")?t`<div><only-desktop></only-desktop></div>`:t`<div class="l-h-scroller">
        <only-desktop></only-desktop>
