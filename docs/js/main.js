(() => {
  ...
  var addToNamedSlot = ({ element }) => {
    const componentWithSlot = useQuery ? queryComponentUseSlot(element) : getAllUserComponentUseNamedSlot({ element });
    if (componentWithSlot.length === 0) return;
    const slots = [...componentWithSlot].map((component) => {
      const slotName = component?.getSlotPosition();
      const slot = useQuery ? querySecificSlot(element, slotName) : getSlotByName({ name: slotName, element });
...
Loading snippet ...</pre
  ...
  function getItems2({ data: data3, staticProps: staticProps2, setState, bindProps }) {
