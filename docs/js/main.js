(() => {
  ...
  var ParallaxClass = class {
     ...
     */
    calcScreenPosition() {
      if (this.screen === window || !this.screen)
        return;
...
Loading snippet ...</pre
  ...
  var setPropFromAncestor = ({ timeline: timeline2, propToFind, activeProp }) => {
    return timeline2.map((item, index) => {
      const { values } = item;
      const newValues = values.map((valueItem) => {
        const { prop, active } = valueItem;
        if (!active || !activeProp.includes(prop))
          return valueItem;
  ...
  var insertNewRow = ({ timeline: timeline2, values, start, end, duration: duration2 }) => {
