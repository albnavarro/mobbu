(() => {
  ...
  var setParent = ({ repeatId, context }) => {
    const parent = (
      /** @type{HTMLElement} */
      context.parentNode
    );
  ...
  var setParent2 = ({ invalidateId, context }) => {
    const parent = (
      /** @type{HTMLElement} */
      context.parentNode
    );
      ...
      class extends HTMLElement {
        constructor() {
          ...
          if (dataset) {
            ...
            setParent2({ invalidateId, context: this });
            (async () => {
              await awaitNextLoop();
              const parent = setParent2({
                invalidateId,
                context: this
              });
              parent?.removeChild(this);
  ...
  var reorderUserPlaceholder = () => {
    const newValues = [...userPlaceholder.values()].sort((a, b) => {
      const posCompare = a.compareDocumentPosition(b);
      if (posCompare & Node.DOCUMENT_POSITION_FOLLOWING || posCompare & Node.DOCUMENT_POSITION_CONTAINED_BY) {
        return -1;
      }
      if (posCompare & Node.DOCUMENT_POSITION_PRECEDING || posCompare & Node.DOCUMENT_POSITION_CONTAINS) {
        return 1;
      }
      if (posCompare & Node.DOCUMENT_POSITION_DISCONNECTED || posCompare & Node.DOCUMENT_POSITION_IMPLEMENTATION_SPECIFIC) {
  ...
  var convertToRealElement = ({ element, content: content2 }) => {
    return new Promise((resolve) => {
...
Loading snippet ...</pre
  ...
  function getItems2({ data: data3, staticProps: staticProps2, setState, bindProps, bindEvents }) {
...
`;
  ...
  var getThirdLevel = ({ repeat, staticProps: staticProps2, delegateEvents }) => {
    return renderHtml`
